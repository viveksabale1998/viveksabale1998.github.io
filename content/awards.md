+++
title = "Education, Awards & CV"
template = "page.html"
aliases = ["cv"]
+++

## Education

**PhD Scholar (Quantum Information & Computation)**  
*Indian Institute of Technology Jodhpur (2021 – Present)*  
- Research Area: Quantum Information, Open Quantum Systems, Non-Markovian Dynamics, and Quantum Machine Learning.  
- Advisors: Prof. Atul Kumar.

**Master of Science in Chemistry**  
*Indian Institute of Technology Madras (2019 – 2021)*  


**Bachelor of Science in Chemistry**  
*Fergusson College, Pune (2016 – 2019)*  

---

## Honors & Awards

**International Travel Grant**  
*Anusandhan National Research Foundation (ANRF), Government of India (2024)*  
- Received competitive travel grant for presenting research at *"Quantum Techniques in Machine Learning (QTML),"* University of Melbourne, Australia.

**Workshop Instructor & Teaching**  
*IIIT Dharwad & QIndia (2024)*  
- Invited to teach hands-on quantum computing and algorithm sessions.

**IBM Quantum Challenge Certificate**  
*IBM Quantum (2021)*  
- Completed competitive challenge covering advanced quantum circuits, error mitigation, and algorithms.

**GATE Qualified**  
*Ministry of Education, Government of India (2021)*  
- Qualified Graduate Aptitude Test in Engineering (Chemistry).

**Diploma Certificate in Chemistry (POCE)**  
*Jawaharlal Nehru Centre for Advanced Scientific Research (JNCASR), Bangalore (2019)*  
- Selected for the prestigious Project Oriented Chemistry Education program.

**Mahatma Jyotirav Phule Gunvant Vidyarthi Scholarship**  
*Savitribai Phule Pune University (2018)*  
- Awarded for academic excellence and top ranking.

---

## Curriculum Vitae (CV)

You can download a PDF copy of my complete curriculum vitae or view it embedded below:

<div style="margin: 1.25rem 0 2rem 0;">
  <a href="https://viveksabale1998.github.io/assets/Vivek_Sabale_CV.pdf" target="_blank" rel="noopener noreferrer" class="skill-btn" style="display: inline-flex; align-items: center; gap: 8px; font-weight: 600; padding: 0.65rem 1.35rem; background: #003687; color: #ffffff; border-radius: 8px; text-decoration: none; box-shadow: 0 4px 12px rgba(0,54,135,0.15);">
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
    Download Full CV (PDF)
  </a>
</div>

{{ pdf_embed(path="assets/Vivek_Sabale_CV.pdf") }}

---

## Contact & Inquiries

For academic collaborations, conference invitations, or research discussions:

* **Email**: [sabale.1@iitj.ac.in](mailto:sabale.1@iitj.ac.in)
* **Google Scholar**: [Vivek Sabale Profile ↗](https://scholar.google.com/citations?user=LdMLDdwAAAAJ&hl=en)
* **GitHub**: [@viveksabale1998 ↗](https://github.com/viveksabale1998)
* **LinkedIn**: [Vivek Sabale ↗](https://www.linkedin.com/in/vivek-sabale/)
